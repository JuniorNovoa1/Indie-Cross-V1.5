...
class TitleState extends MusicBeatState
{
	...
	{
        // MusicBeatState.switchState(new Start());
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			//the code above loads the affect in and sets it to the default -thatblockboi
		...
		Play = new FlxSprite(690, 580);
		Play.frames = Paths.getSparrowAtlas('title/Playbutton');
		Play.animation.addByPrefix('Idle', 'Button instance', 24); 
		Play.animation.play('Idle');
		Play.scale.set(1.0, 1.0);
		Play.antialiasing = ClientPrefs.globalAntialiasing;
		add(Play);
		//scale makes it bigger, angularVelocity makes it spin Negative = spinLeft NonNegative = spinRight -thatblockboi
		var text:FlxSprite = new FlxSprite(730, 590).loadGraphic(Paths.image('title/PlayText'));
		//text.screenCenter();
		text.antialiasing = ClientPrefs.globalAntialiasing;
		add(text);

		CupCircle = new FlxSprite(-150, -100).loadGraphic(Paths.image('title/CupCircle'));
		CupCircle.antialiasing = ClientPrefs.globalAntialiasing;
		CupCircle.angularVelocity = 50;
		CupCircle.scale.set(0.8, 0.8);
		add(CupCircle);

		SansCircle = new FlxSprite(500, -100).loadGraphic(Paths.image('title/SansCircle'));
		SansCircle.antialiasing = ClientPrefs.globalAntialiasing;
		SansCircle.screenCenter(X);
		SansCircle.angularVelocity = -70;
		SansCircle.scale.set(0.9, 0.9);
		add(SansCircle);

		BendyCircle = new FlxSprite(550, -30).loadGraphic(Paths.image('title/BendyCircle'));
		BendyCircle.antialiasing = ClientPrefs.globalAntialiasing;
		BendyCircle.angularVelocity = 70;
		BendyCircle.scale.set(0.9, 0.9);
		add(BendyCircle);

		BF = new FlxSprite(695, 190); //how did this take me 2 hours to figure out
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(2, function(tmr:FlxTimer)
				{
					...
					if (mustUpdate) {
						...
						transIn = FlxTransitionableState.defaultTransIn;
	                	transOut = FlxTransitionableState.defaultTransOut;
						//makes the effect more stable -thahtblockboi
